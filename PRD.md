...
本文档旨在详细描述OKEx订单簿实时分析系统的业务需求、功能特性和性能要求。该系统是一个**独立的中间件服务**，通过WebSocket实时获取并分析订单簿数据，采用**以数据为中心的架构风格**，将所有分析结果持久化到Redis中，为下游应用提供标准化、低延迟的实时市场数据分析服务，为产品开发、测试和验收提供明确指导。
...
- **中间件服务**：独立运行的软件组件，为其他应用提供特定功能，如数据处理、通信等
- **以数据为中心架构**：一种架构风格，数据是系统的核心，所有处理逻辑围绕数据的存储、检索和处理设计
- **Redis**：高性能的键值对存储数据库，用于缓存、消息队列和实时数据存储
- **数据持久化**：将内存中的数据保存到稳定存储介质（如Redis）的过程

## 2. 产品目标

### 2.1 核心目标

作为独立的中间件服务，通过WebSocket实时获取并分析OKEx交易平台的订单簿数据，采用以数据为中心的架构风格，将分析结果持久化到Redis中，为下游应用提供高可用、低延迟的实时市场数据分析服务。

### 2.2 具体目标

- 实时获取并处理OKEx交易平台的订单簿数据
- 支持最多10个交易对的同时监控和分析
- 提供多种实时数据分析指标，并将结果持久化到Redis中
- 及时识别市场异常情况，并将预警信息存储到Redis中
- 为下游应用提供稳定、高效的数据访问接口
...
- 计算得到的支撑位与阻力位结果将被持久化到Redis中，键名为`analysis:support_resistance:{instrument_id}`
...
- 分析得到的大额订单分布情况和市场情绪指标将被持久化到Redis中，键名为`analysis:large_orders:{instrument_id}`
...
- 检测到的深度异常波动信息将被持久化到Redis中，键名为`analysis:depth_anomaly:{instrument_id}`
...
- 预警应提示用户当前市场流动性风险增加
- 检测到的流动性萎缩预警信息将被持久化到Redis中，键名为`analysis:liquidity_shrink:{instrument_id}`
...
作为以数据为中心的架构，系统将所有实时分析结果持久化到Redis中，采用以下数据存储策略：

#### 3.5.1 Redis数据结构设计

| 数据类型 | 键名模式 | 用途 | 示例值 |
|---------|---------|------|--------|
| Hash | `orderbook:{instrument_id}` | 存储最新订单簿快照 | `{"asks": [...], "bids": [...], "timestamp": 1234567890}` |
| Hash | `analysis:support_resistance:{instrument_id}` | 存储支撑位与阻力位计算结果 | `{"supports": ["45000", "44500"], "resistances": ["46000", "46500"]}` |
| Hash | `analysis:large_orders:{instrument_id}` | 存储大额订单分布分析结果 | `{"bullPower": 123456, "bearPower": 789012, "sentiment": 0.25}` |
| Hash | `analysis:depth_anomaly:{instrument_id}` | 存储深度异常波动信息 | `{"is_anomaly": true, "direction": "increase", "strength": 2.5}` |
| Hash | `analysis:liquidity_shrink:{instrument_id}` | 存储流动性萎缩预警信息 | `{"is_alert": true, "level": "medium", "current_liquidity": 12345}` |


#### 3.5.2 数据访问模式

- **实时写入**：所有分析结果在计算完成后立即写入Redis对应的数据结构
- **批量更新**：对于高频更新的数据（如订单簿快照），采用增量更新策略减少网络开销

- **查询优化**：使用Redis的Hash数据结构支持快速的字段级查询和更新

#### 3.5.3 数据过期策略

- 订单簿快照：无过期时间，始终保持最新状态
- 分析结果：无过期时间，始终保持最新状态


#### 3.5.4 高可用设计

- Redis采用主从复制架构，确保数据的可靠性和可用性
- 系统实现自动重连机制，当Redis连接中断时能够自动恢复
- 关键数据采用异步写入策略，避免Redis故障影响系统核心功能

### 3.6 系统数据流

作为以数据为中心的架构，系统的核心是数据流的处理和存储。以下是系统的整体数据流图：

```mermaid
flowchart TD
    A[OKEx WebSocket数据源] --> B[数据获取与标准化模块]
    B --> C[实时订单簿维护]
    C --> D[支撑位与阻力位计算]
    C --> E[大额订单分布分析]
    C --> F[深度异常波动检测]
    C --> G[流动性萎缩预警]
    D --> H[Redis Hash存储: analysis:support_resistance:{instrument_id}]
    E --> I[Redis Hash存储: analysis:large_orders:{instrument_id}]
    F --> J[Redis Hash存储: analysis:depth_anomaly:{instrument_id}]
    G --> K[Redis Hash存储: analysis:liquidity_shrink:{instrument_id}]

    H --> O[下游应用]
    I --> O
    J --> O
    K --> O
    C --> P[Redis Hash存储: orderbook:{instrument_id}]
    P --> O
```

#### 3.6.1 数据流详细说明

1. **数据获取与标准化**：系统通过WebSocket连接到OKEx平台，获取订单簿快照和增量更新数据，并进行标准化处理。
2. **实时订单簿维护**：系统维护每个交易对的最新订单簿状态，支持增量更新和快照恢复。
3. **实时分析**：基于维护的订单簿数据，系统并行执行多种分析算法：
   - 支撑位与阻力位计算
   - 大额订单分布分析
   - 深度异常波动检测
   - 流动性萎缩预警

4. **数据存储**：所有分析结果和最新订单簿快照都被持久化到Redis中，使用不同的数据结构和键名模式。
5. **数据访问**：下游应用通过Redis API访问存储的分析结果和订单簿数据，实现实时监控和交易决策。

#### 3.6.2 数据流特点

- **以数据为中心**：所有处理逻辑围绕数据的获取、处理和存储设计
- **并行处理**：多种分析算法可以并行执行，提高系统效率
- **实时性**：从数据获取到存储的整个过程控制在50毫秒以内
- **可扩展性**：新的分析算法可以轻松添加到数据流中
- **松耦合**：下游应用通过Redis与系统解耦，提高系统的灵活性和可维护性
